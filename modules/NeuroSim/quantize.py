...
try:
    ...
    quant_nn = None
    calib = None
    QuantDescriptor = None
    quant_modules = None
    cim = None
    print("[Warning] pytorch_quantization not available in quantize.py, hardware-only mode active")

from collections import namedtuple

# Definition of the named tuple used to store mapping of quantized modules
...
def quantize_model(args, criterion, data_loader_quant, data_loader_test):
    """
    Load and (optionally) quantize a model.

    When pytorch_quantization is available the full CIM-aware quantization
    pipeline runs.  When it is not available (HAS_PTQ=False) we fall back to
    loading the plain pretrained model and returning it directly — which is
    sufficient for hardware topology evaluation via NeuroSim.
    """

    # ── Step 1: build / load the model ───────────────────────────────────────
    ...
    # Load saved weights for non-ImageNet models
    if args.dataset != 'imagenet':
        print(f"\nLoading pretrained model {saved_model}...")
        state_dict = torch.load(saved_model, map_location='cpu')
        # Strip 'module.' prefix added by nn.DataParallel if present
    ...
    # ── No PTQ available: return plain model for hardware-only evaluation ─────
    if not HAS_PTQ:
        print("\n[Hardware-only mode] Skipping quantization — returning plain model.")
        print("Evaluating baseline model accuracy...\n")
        baseline_accuracy = evaluate(model, args, criterion, data_loader_test,
                                     num_batches=1, print_freq=1)
        print(f"Baseline accuracy: {baseline_accuracy:.2f}%")
        return model

    # ── Full PTQ pipeline (only reached when HAS_PTQ=True) ───────────────────
    hardware = args.hardware
    args.hardware = False

    quant_modules.initialize(
        float_module_list=['Conv2d', 'Linear', 'AvgPool2d', 'MaxPool2d', 'AdaptiveAvgPool2d'],
        custom_quant_modules=cim_quant_map
    )

    if args.input_calib_method  == 'histogram': args.input_axis  = None
    if args.weight_calib_method == 'histogram': args.weight_axis = None
    if args.adc_calib_method    == 'histogram': args.adc_axis    = None
    ...
    input_desc     = QuantDescriptor(calib_method=args.input_calib_method,
                                     ...
                                     axis=args.input_axis, unsigned=False)
    weight_desc    = QuantDescriptor(calib_method=args.weight_calib_method,
                                     num_bits=args.weight_precision,
                                     fake_quant=args.fake_quant,
                                     axis=args.weight_axis, unsigned=False)
    adc_quant_desc = QuantDescriptor(calib_method=args.adc_calib_method,
                                     num_bits=args.adc_precision,
                                     fake_quant=args.fake_quant,
                                     axis=args.adc_axis, unsigned=True)
    ...
    layer_quant = layers[1:]   # skip first layer
    ...
    collect_stats(model, layers, data_loader_quant, args.gpu,
                  quant_mode='iw', num_batches=2)

    strict = args.model != 'swin_t'
    print('\nComputing amax... \n')
    compute_amax(model, args, quant_mode='iw', layers=layers,
                 method="percentile", percentile=99.9999, strict=strict)
    print("Finished quantizing inputs and weights.")

    print("Evaluating input and weight quantized model...\n")
    iw_quant_accuracy = evaluate(model, args, criterion, data_loader_test,
                                 num_batches=1, print_freq=1)
...
def evaluate(model, args, criterion, data_loader, num_batches, print_freq=100,
             log_suffix="", filename=None):
    ...
    with torch.no_grad():
        ...
        for i, (images, labels) in tqdm(enumerate(data_loader), total=num_batches):
            ...
            print('Test Accuracy of the model on {} test images: {} %'.format(
                (i + 1) * args.batch_size, 100 * correct / total))
        ...
        total_loss /= test_count
        args.logger('Test Accuracy of the model on the 50000 test images: {} %'.format(
            100 * correct / total))
...
def collect_stats(model, layer_quant, data_loader, gpu, quant_mode='iw', num_batches=2):
    """Feed data to the network and collect statistics for calibration."""
    if not HAS_PTQ:
        return
...
def compute_amax(model, args, layers, quant_mode='iw', **kwargs):
    """Load calibration results and compute amax for quantized layers."""
    if not HAS_PTQ:
        return
        ...
        if isinstance(module, macro.CIM):
            if hasattr(module, 'weight'):
                if module.mem_sampling_seed.numel() != 0:
                    module.name = name
                    module.mem_sampling_seed = torch.randint(
                        1, 1000, module.mem_sampling_seed.size())